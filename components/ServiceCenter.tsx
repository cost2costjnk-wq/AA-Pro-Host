        ...
        <div className="p-4 lg:p-8 max-w-7xl mx-auto space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-3xl p-5 shadow-sm flex flex-col lg:flex-row gap-4">
                <div className="flex-1 relative">
                    ...
                    <input 
                        type="text" 
                        placeholder="Search ticket, customer or model..." 
                        className="w-full pl-10 pr-4 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-sm font-medium focus:ring-2 focus:ring-brand-500/50 outline-none dark:text-white transition-all"
                        value={searchTerm}
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div className="relative min-w-[160px]">
                        ...
                        <select 
                            className="w-full pl-9 pr-10 py-2.5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-sm font-bold text-gray-700 dark:text-gray-200 outline-none appearance-none focus:ring-2 focus:ring-brand-500/50"
            ...
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-[2rem] overflow-hidden shadow-sm">
                <div className="p-5 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50/30 dark:bg-gray-900/20">
                    ...
                    <span className="text-[10px] font-black text-brand-600 uppercase bg-brand-50 dark:bg-brand-900/30 px-2 py-1 rounded-lg">
                            ...
                            {filteredJobs.map(job => (
                                <tr key={job.id} className="hover:bg-gray-50/80 dark:hover:bg-gray-700/30 group transition-all">
                                    <td className="px-8 py-5 text-center">
                                        ...
                                        <div className="text-[9px] font-bold text-gray-400 dark:text-gray-500 mt-1 uppercase flex items-center justify-center gap-1">
                                            <Calendar className="w-2.5 h-2.5" /> {formatNepaliDate(job.date)}
                                        </div>
                                    </td>
                                    <td className="px-8 py-5">
                                        <div className="flex items-center gap-3">
                                            <div className={`p-2 rounded-xl ${job.customerId ? 'bg-brand-50 dark:bg-brand-900/20 text-brand-600' : 'bg-gray-100 dark:bg-gray-700 text-gray-400'}`}>
                                    ...
                                    <td className="px-8 py-5">
                                        <div className="flex items-center gap-2">
                                            <Smartphone className="w-4 h-4 text-gray-300 dark:text-gray-600" />
                                            <span className="font-semibold text-gray-700 dark:text-gray-300">{job.deviceModel}</span>
                                        </div>
                                    ...
                                    <td className="px-8 py-5 text-right">
                                        <div className="font-black text-gray-900 dark:text-white">{formatCurrency(job.estimatedCost)}</div>
                                        {job.advanceAmount > 0 && <div className="text-[10px] text-emerald-600 dark:text-emerald-400 font-bold uppercase">Adv: {formatCurrency(job.advanceAmount)}</div>}
                                    </td>
                                    <td className="px-8 py-5 text-center">
                                        <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                                            ...
                                            <button 
                                                ...
                                                className="p-2 text-gray-400 hover:text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-900/30 rounded-xl transition-all"
                                                title="Edit Ticket"
                                            >
                ...
                {filteredJobs.length === 0 && (
                    <div className="py-24 text-center flex flex-col items-center gap-3 bg-gray-50/50 dark:bg-gray-900/10">
                        <div className="w-16 h-16 bg-gray-100 dark:bg-gray-800 rounded-3xl flex items-center justify-center mb-2">
                            <ClipboardList className="w-8 h-8 text-gray-300 dark:text-gray-600" />
                        </div>
                        <p className="text-sm font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">No service records matching current filters</p>
                        {hasActiveFilters && (
                            <button onClick={handleClearFilters} className="text-brand-600 font-bold text-xs uppercase hover:underline">Reset All Filters</button>
                        )}
                    </div>
                )}
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-[2.5rem] w-full max-w-2xl shadow-2xl flex flex-col max-h-[95vh] overflow-hidden border border-gray-100 dark:border-gray-700">
                <div className="px-10 py-8 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50/50 dark:bg-gray-900/50">
                    <div>
                        ...
                        <p className="text-xs text-gray-400 dark:text-gray-500 font-bold uppercase tracking-widest mt-1">Repair Registration Form</p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-10 space-y-8 custom-scrollbar">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="relative" ref={dropdownRef}>
                                ...
                                <div className="relative">
                                    ...
                                    />
                                    {formData.customerId && (
                                        <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1.5 px-2 py-1 bg-brand-50 dark:bg-brand-900/30 text-brand-600 dark:text-brand-400 rounded-lg text-[9px] font-black uppercase">
                                ...
                                {showNameDropdown && nameSearch && (
                                    <div className="absolute z-50 top-full left-0 w-full mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-2xl overflow-hidden animate-in slide-in-from-top-2 duration-200">
                                        <div className="bg-gray-50 dark:bg-gray-900 px-4 py-2 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
                                        ...
                                        <div className="max-h-48 overflow-y-auto">
                                            {filteredParties.map((p, idx) => (
                                                <div 
                                                    key={p.id} 
                                                    className={`p-4 border-b last:border-0 dark:border-gray-700 cursor-pointer flex justify-between items-center transition-colors ${highlightedNameIndex === idx ? 'bg-brand-50 dark:bg-brand-900/20' : 'hover:bg-gray-50 dark:hover:bg-gray-700'}`}
                                                    onClick={() => handlePartySelect(p)}
                                                >
                                                    <div>
                                                        <span className="font-bold text-gray-800 dark:text-gray-200 text-sm block">{p.name}</span>
                                                        <span className="text-[10px] text-gray-400 dark:text-gray-500 font-medium">{p.phone || 'No Phone'}</span>
                                                    </div>
                                                    <ArrowRight className={`w-4 h-4 transition-all ${highlightedNameIndex === idx ? 'text-brand-500 translate-x-0' : 'text-gray-200 dark:text-gray-700 -translate-x-2'}`} />
                                                </div>
                                            ))}
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-2">Primary Phone</label>
                                <input 
                                    className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 text-sm font-bold outline-none focus:ring-4 focus:ring-brand-500/10 focus:border-brand-500 dark:text-white transition-all" 
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-2">Device Model</label>
                                <input className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 text-sm font-bold outline-none focus:ring-4 focus:ring-brand-500/10 focus:border-brand-500 dark:text-white transition-all" placeholder="e.g. iPhone 15 Pro Max" value={formData.deviceModel} onChange={e => setFormData({...formData, deviceModel: e.target.value})} />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-2">IMEI / Serial Number</label>
                                <input className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 text-sm font-bold outline-none focus:ring-4 focus:ring-brand-500/10 focus:border-brand-500 dark:text-white transition-all" placeholder="Enter device identifier..." value={formData.deviceImei} onChange={e => setFormData({...formData, deviceImei: e.target.value})} />
                            </div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select 
                                    className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 text-sm font-bold outline-none focus:ring-4 focus:ring-brand-500/10 focus:border-brand-500 dark:text-white transition-all appearance-none" 
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-2">Device Password / Pattern</label>
                                <input className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 text-sm font-bold outline-none focus:ring-4 focus:ring-brand-500/10 focus:border-brand-500 dark:text-white transition-all" placeholder="Pattern or PIN..." value={formData.devicePassword} onChange={e => setFormData({...formData, devicePassword: e.target.value})} />
                            </div>
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-2">Problem Description</label>
                            <textarea 
                                className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 text-sm font-bold outline-none focus:ring-4 focus:ring-brand-500/10 focus:border-brand-500 dark:text-white transition-all resize-none min-h-[100px]" 
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="p-6 bg-brand-50/50 dark:bg-brand-900/10 rounded-3xl border border-brand-100/50 dark:border-brand-800/50">
                                <label className="block text-[10px] font-black text-brand-600 dark:text-brand-400 uppercase mb-2">Total Estimate (Rs.)</label>
                                <input type="number" className="w-full bg-white dark:bg-gray-900 border border-brand-100 dark:border-brand-800 rounded-xl p-3 text-xl font-black text-brand-700 dark:text-white outline-none" value={formData.estimatedCost} onChange={e => setFormData({...formData, estimatedCost: Number(e.target.value)})} />
                            </div>
                            <div className="p-6 bg-emerald-50/50 dark:bg-emerald-900/10 rounded-3xl border border-emerald-100/50 dark:border-emerald-800/50">
                                <label className="block text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase mb-2">Advance Amount (Rs.)</label>
                                <input type="number" className="w-full bg-white dark:bg-gray-900 border border-emerald-100 dark:border-emerald-800 rounded-xl p-3 text-xl font-black text-emerald-700 dark:text-white outline-none" value={formData.advanceAmount} onChange={e => setFormData({...formData, advanceAmount: Number(e.target.value)})} />
                            </div>
                        </div>
                    </div>
                </div>

                <div className="px-10 py-8 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex justify-end gap-4">
                    <button onClick={onClose} className="px-8 py-3 text-gray-500 dark:text-gray-400 font-bold uppercase text-xs tracking-widest hover:text-gray-700 dark:hover:text-gray-200 transition-colors">Discard</button>
